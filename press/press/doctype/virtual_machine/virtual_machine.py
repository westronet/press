...
class VirtualMachine(Document):
	...
	def _create_snapshots_aws(self):
		response = self.client().create_snapshots(
			...
			Description=f"Westro Cloud - {self.name} - {frappe.utils.now()}",
			TagSpecifications=[
				{
					"ResourceType": "snapshot",
					"Tags": [
						{"Key": "Name", "Value": f"Westro Cloud - {self.name} - {frappe.utils.now()}"}
					],
				},
			],
		)
				...
				snapshot = (
					...
					.create_boot_volume_backup(
						CreateBootVolumeBackupDetails(
							boot_volume_id=volume.volume_id,
							type="INCREMENTAL",
							display_name=f"Westro Cloud - {self.name} - {volume.name} - {frappe.utils.now()}",
						)
					)
					.data
				)
