...
class Cluster(Document):
	...
	def provision_on_aws_ec2(self):
		...
		response = client.create_vpc(
			...
			TagSpecifications=[
				{
					"ResourceType": "vpc",
					"Tags": [{"Key": "Name", "Value": f"Westro Cloud - {self.name}"}],
				},
			],
		...
		response = client.create_subnet(
			TagSpecifications=[
				{
					"ResourceType": "subnet",
					"Tags": [{"Key": "Name", "Value": f"Westro Cloud - {self.name} - Public Subnet"}],
				},
			],
			...
			TagSpecifications=[
				{
					...
					"Tags": [
						{"Key": "Name", "Value": f"Westro Cloud - {self.name} - Internet Gateway"},
					],
				},
			],
		)
		...
		client.create_tags(
			...
			Tags=[{"Key": "Name", "Value": f"Westro Cloud - {self.name} - Route Table"}],
		)
		...
		client.create_tags(
			...
			Tags=[{"Key": "Name", "Value": f"Westro Cloud - {self.name} - Network ACL"}],
		)

		response = client.create_security_group(
			GroupName=f"Westro Cloud - {self.name} - Security Group",
			...
			TagSpecifications=[
				{
					...
					"Tags": [
						{"Key": "Name", "Value": f"Westro Cloud - {self.name} - Security Group"},
					],
				},
			],
		)
	...
	def create_proxy_security_group(self):
		...
		response = client.create_security_group(
			GroupName=f"Westro Cloud - {self.name} - Proxy - Security Group",
			...
			TagSpecifications=[
				{
					"ResourceType": "security-group",
					"Tags": [
						{"Key": "Name", "Value": f"Westro Cloud - {self.name} - Proxy - Security Group"},
					],
				},
			],
		)
	...
	def provision_on_oci(self):
		...
		vcn = vcn_client.create_vcn(
			CreateVcnDetails(
				...
				display_name=f"Westro Cloud - {self.name}",
		...
		security_group = vcn_client.create_network_security_group(
			CreateNetworkSecurityGroupDetails(
				compartment_id=self.oci_tenancy,
				display_name=f"Westro Cloud - {self.name} - Security Group",
				vcn_id=self.vpc_id,
			)
		).data
		...
		proxy_security_group = vcn_client.create_network_security_group(
			CreateNetworkSecurityGroupDetails(
				compartment_id=self.oci_tenancy,
				display_name=f"Westro Cloud - {self.name} - Proxy - Security Group",
				vcn_id=self.vpc_id,
			)
		).data
		...
		subnet = vcn_client.create_subnet(
			CreateSubnetDetails(
				compartment_id=self.oci_tenancy,
				display_name=f"Westro Cloud - {self.name} - Public Subnet",
				vcn_id=self.vpc_id,
				cidr_block=self.subnet_cidr_block,
		...
		internet_gateway = vcn_client.create_internet_gateway(
			CreateInternetGatewayDetails(
				compartment_id=self.oci_tenancy,
				display_name=f"Westro Cloud - {self.name} - Internet Gateway",
				is_enabled=True,
				vcn_id=self.vpc_id,
			)
		).data
